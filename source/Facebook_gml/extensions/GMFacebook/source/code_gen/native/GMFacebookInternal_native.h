...
namespace gm::wire::codec
{
    ...
    inline gm_structs::FacebookEventParameterValue readValue<gm_structs::FacebookEventParameterValue>(gm::byteio::BufferReader& _buf)
    {
        ...
        obj.key = gm::wire::codec::readValue<std::int32_t>(_buf);
...
void fb_graph_request(std::string_view graph_path, const std::vector<gm_enums::FacebookHttpMethod>& method, const std::vector<gm_structs::FacebookNamedValue>& parameters, const gm::wire::GMFunction& callback);
void fb_dialog(std::string_view link_url, const gm::wire::GMFunction& callback);
bool fb_send_event(const std::vector<gm_enums::FacebookAppEvent>& event, double value, const std::vector<gm_structs::FacebookEventParameterValue>& parameters);
